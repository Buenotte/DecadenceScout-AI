...
│   ├── historian_agent.py         # Agent 3: Historiker-Investigator (BOE, DOGV, Hemerotheken via Kimi K3 API)
...
| **`openai`** | Offizielles Python SDK für OpenAI-kompatible Cloud-API Schnittstellen. | `backend/model_router.py` | Steuert die REST-API Aufrufe an die chinesischen Top-Modelle **DeepSeek-V4 API** (Orchestrator) und **Kimi K3 API** (Moonshot AI mit 2M+ Tokens & Context Caching). |
...
### 🟣 1. Kimi K3 API Key (`api.moonshot.cn`) - 2 Mio.+ Tokens Long-Context & Caching
...
class ModelRouter:
    ...
    Anbindung an DeepSeek-V4 API, Kimi K3 API (Moonshot AI 2M+ Context & Caching), GLM-5 API (Zhipu) und Qwen 3 Cloud API (Alibaba).
    """
    def __init__(self):
    ...
    def route_request(self, agent_name, prompt, system_prompt=""):
        ...
        if agent_name == "HistorianAgent":
            return self.call_kimi_k3_long_context(prompt, system_prompt)
    ...
    def call_kimi_k3_long_context(self, prompt, system_prompt=""):
        ...
        try:
            res = self.kimi_client.chat.completions.create(
                model="kimi-k3",
                messages=[
                    {"role": "system", "content": system_prompt or "Du bist Kimi K3 for Long Context & Caching."},
                    {"role": "user", "content": prompt}
                ]
            )
            return {"status": "success", "provider": "Kimi K3 API (Moonshot AI 2M+ Context & Caching)", "response": res.choices[0].message.content}
        except Exception as e:
            return self.call_glm5_api(prompt, system_prompt)
...
class HistorianInvestigatorAgent:
    ...
    def research_location(self, name, lat, lon):
        print(f"[Agent 3 - Historiker Kimi K3 2M+ API] Deep Research für '{name}' ({lat}, {lon})...")
        system_prompt = "Du bist Kimi K3, ein historischer Investigator für DekadenzScout AI."
...
const SAMPLE_SPOTS = [
  { id: 1, name: "Sanatorio de Aigües de Busot", lat: 38.5031, lon: -0.4132, province: "Alicante", ndvi: 0.58, ndwi: 0.12, type: "Sanatorium / Hotel", status: "KNOWN_HISTORIC_SITE", year: 1898, history: "Ehemaliges Luxus-Thermalbad, später Tuberkulose-Klinik." },
      ...
      <aside className="w-96 flex flex-col border-r border-amber-500/20 bg-slate-900/90 backdrop-blur-md p-5 z-20 overflow-y-auto">
        ...
        <div className="glass-panel p-3.5 rounded-xl mb-6 text-xs space-y-2">
          ...
          <div className="text-slate-300 space-y-1 text-[11px]">
            ...
            <div className="flex justify-between"><span>Archive Mining:</span> <span className="text-cyan-400">Kimi K3 (2M+ Caching)</span></div>
          </div>
        </div>
